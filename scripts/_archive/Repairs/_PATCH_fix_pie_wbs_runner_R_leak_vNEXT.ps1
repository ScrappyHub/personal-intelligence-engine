...
# Replace ONLY the stable inner-fragment; single quotes prevent $ expansion.
$needle = 'R.Add(''[void]$R.Add'
$repl   = 'R.Add(''[void]$md.Add'
$txt2 = $txt.Replace($needle,$repl)
if($txt2 -eq $txt){
  $ls = $txt -split "`n"
  $sn = New-Object System.Collections.Generic.List[string]
  [void]$sn.Add("PATCH_FAIL: needle not found: " + $needle)
  [void]$sn.Add("backup=" + $bak)
  [void]$sn.Add("HINT lines containing `$R/`$md/R.Add/md.Add:")
  for($i=0;$i -lt $ls.Length;$i++){
    $l=$ls[$i]
    if($l -match "\$R" -or $l -match "\$md" -or $l -match "(?i)\bR\.Add\b" -or $l -match "(?i)\bmd\.Add\b"){
...
Write-Host ("PATCH_OK: fixed WBS runner inner R leak (backup=" + $bak + ")") -ForegroundColor Green
