...
# Robust minimal substring replace (single quotes = no $ expansion)
...
if($txt2 -eq $txt){
  ...
  for($i=0;$i -lt $ls.Length;$i++){
    $l=$ls[$i]
    if($l -match "(?i)\bmd\.Add\b|\bR\.Add\b|\$R\b|\$md\b"){ [void]$sn.Add(("L{0}: {1}" -f ($i+1), $l.TrimEnd())) }
  }
...
Write-Host ("PATCH_OK: runner inner emitted typo fixed (backup=" + $bak + ")") -ForegroundColor Green
