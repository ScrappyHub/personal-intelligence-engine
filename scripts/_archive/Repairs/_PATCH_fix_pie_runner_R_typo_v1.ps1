...
# Fix the known typo that references $R at runtime
$txt2 = $txt.Replace($bad,$good)
if($txt2 -eq $txt){ Die ("PATCH_FAIL: did not find expected typo line. backup=" + $bak) }
Parse-GateText $txt2
Write-Utf8NoBomLf $target $txt2
Parse-GateText (Read-Utf8NoBom $target)
Write-Host ("PATCH_OK: fixed $R typo in runner (backup=" + $bak + ")") -ForegroundColor Green
