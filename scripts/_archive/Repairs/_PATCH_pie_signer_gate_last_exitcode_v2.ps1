...
$sent = "# --- PIE_PATCH_SIGNER_LASTEXITCODE_GATE_V2 ---"
...
if ($capIdx -lt 0 -or [string]::IsNullOrWhiteSpace($capVar)) {
  $sn=@(); $sn += "patch_fail: could not find capture sign line ($"+"signOut = & $ssh -Y sign ... 2>&1).";
  $sn += ("backup=" + $bak);
  $sn += "DIAG: candidate lines:";
  for($j=0;$j -lt $ls.Length;$j++){ $l2=$ls[$j]; if($l2 -match "(?i)\bssh\b|\b-Y\b|\bsign\b|2>&1|signOut|LASTEXITCODE"){ $sn += ("L" + ($j+1) + ": " + $l2.TrimEnd()) } }
  Die (($sn -join "`n"))
}
...
if ($ifStart -lt 0) {
  $sn=@(); $sn += ("patch_fail: could not find if(" + $capVar + "){...} block after capture line.");
  $sn += ("backup=" + $bak);
