...
$sent = "# --- PIE_PATCH_SIGNER_EXITCODE_GATE_V4 ---"
if ($txt -like ("*" + $sent + "*")) { Write-Host ("OK: signer already patched (backup=" + $bak + ")") -ForegroundColor Yellow; return }

# Find a block that: captures output from ssh-keygen sign into $var, then does if($var){ throw/Die }
# We patch ONLY that if($var){...} to instead gate on $LASTEXITCODE.
$re = '(?is)(\$\w+)\s*=\s*&[^\r\n]*ssh-keygen[^\r\n]*-Y[^\r\n]*\bsign\b[^\r\n]*[\r\n]+(?:[^\r\n]*[\r\n]+){0,20}?if\s*\(\s*\1\s*\)\s*\{[\s\S]{0,2500}?\}'
$m = [regex]::Match($txt, $re)
if (-not $m.Success) {
  # Fallback: capture var from ANY "& ... -Y ... sign" line (ssh-keygen may be stored in $ps or $ssh var)
  $re2 = '(?is)(\$\w+)\s*=\s*&[^\r\n]*\b-Y\b[^\r\n]*\bsign\b[^\r\n]*[\r\n]+(?:[^\r\n]*[\r\n]+){0,20}?if\s*\(\s*\1\s*\)\s*\{[\s\S]{0,2500}?\}'
...
$ifRe = '(?is)if\s*\(\s*' + [regex]::Escape($var) + '\s*\)\s*\{[\s\S]{0,2500}?\}'
$newIf = @(
  $sent,
