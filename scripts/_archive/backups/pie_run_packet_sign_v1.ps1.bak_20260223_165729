...
$packetId = (Get-Content -LiteralPath $pidTxt -ErrorAction Stop | Select-Object -First 1).Trim()
if ($packetId -notmatch "^[0-9a-f]{64}$") { Die ("bad_packet_id_txt: " + $packetId) }
...
$e = @{ schema="sig_envelope.v1"; purpose="pie.run_packet"; packet_id=$packetId; namespace=$Namespace; principal=$Principal; manifest_sha256=$mHash; packet_id_txt_sha256=$pidHash; time_utc=(Get-Date).ToUniversalTime().ToString("o") }
...
# --- PIE_PATCH_SIGNER_EXITCODE_GATE_V6 ---
# --- PIE_PATCH_SIGNER_SSHKEYGEN_STARTPROCESS_V1 ---
$tmpOut = $envPath + ".sshkeygen.out.txt"
$tmpErr = $envPath + ".sshkeygen.err.txt"
if (Test-Path -LiteralPath $tmpOut -PathType Leaf) { Remove-Item -LiteralPath $tmpOut -Force -ErrorAction SilentlyContinue }
if (Test-Path -LiteralPath $tmpErr -PathType Leaf) { Remove-Item -LiteralPath $tmpErr -Force -ErrorAction SilentlyContinue }
$args = @("-Y","sign","-f",$key,"-n",$Namespace,"-I",$Principal,$envPath)
$proc = Start-Process -FilePath $ssh -ArgumentList $args -NoNewWindow -Wait -PassThru -RedirectStandardOutput $tmpOut -RedirectStandardError $tmpErr
$signOut = @()
if (Test-Path -LiteralPath $tmpOut -PathType Leaf) { $signOut += @(@(Get-Content -LiteralPath $tmpOut -Encoding UTF8)) }
if (Test-Path -LiteralPath $tmpErr -PathType Leaf) { $signOut += @(@(Get-Content -LiteralPath $tmpErr -Encoding UTF8)) }
...
$msg = "OK: packet signed: " + $packetId + " env=" + $envPath + " sig=" + $sigPath
