param(
  ...
  # A model id sealed under registry\models\<ModelId>\model_manifest.v1.json and served by the
  # local llama.cpp server. Required for the positive (real-generation) check.
...
# Certification test trio for the llama.cpp engine adapter (engine/adapters/llamacpp).
# Emits SELFTEST_PIE_ENGINE_LLAMACPP_V1_GREEN only when the negative + binding checks pass
# AND a real generation was sealed. Until green on the target host, status stays "scaffold".

function Die([string]$Message){ throw ("SELFTEST_ENGINE_LLAMACPP_FAIL: " + $Message) }
...
Write-Host "PIE_ENGINE_LLAMACPP_SELFTEST_START" -ForegroundColor DarkCyan

# --- Check 1 (negative): server down must fail closed, never stub, never ledger. ---
$before = Get-LedgerCount
$failed = $false
$captured = ""
try {
  $env:PIE_LLAMACPP_URL = "http://127.0.0.1:59998/completion"  # unused dead port
  $captured = (& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $runScript `
    -RepoRoot $RepoRoot -ModelId ($(if($ModelId){$ModelId}else{"selftest-missing"})) -Prompt "negative server-down" -Backend llamacpp 2>&1 | Out-String)
  if($LASTEXITCODE -ne 0){ $failed = $true }
}
catch { $failed = $true; $captured = $_.Exception.Message }
finally { Remove-Item Env:\PIE_LLAMACPP_URL -ErrorAction SilentlyContinue }

if(-not $failed){ Die "SERVER_DOWN_DID_NOT_FAIL_CLOSED" }
if($captured -match 'PIE_STUB_OUTPUT'){ Die "SERVER_DOWN_FELL_BACK_TO_STUB" }
$after = Get-LedgerCount
if($after -ne $before){ Die "SERVER_DOWN_APPENDED_LEDGER" }
Write-Host "  check1_negative_server_down: OK" -ForegroundColor Green
...
try {
  $bindingOut = (& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $runScript `
    -RepoRoot $RepoRoot -ModelId "selftest-unsealed-model-xyz" -Prompt "binding" -Backend llamacpp 2>&1 | Out-String)
...
# --- Check 3 (positive): real, non-stub generation is produced, ledgered, and artifacted. ---
if($SkipPositive){
  Write-Host "PIE_ENGINE_LLAMACPP_SELFTEST_INCONCLUSIVE (positive skipped; NOT green)" -ForegroundColor Yellow
  return
}
if([string]::IsNullOrWhiteSpace($ModelId)){
  Write-Host "PIE_ENGINE_LLAMACPP_SELFTEST_INCONCLUSIVE (no -ModelId; positive not run; NOT green)" -ForegroundColor Yellow
  return
}
...
Write-Host "SELFTEST_PIE_ENGINE_LLAMACPP_V1_GREEN" -ForegroundColor Green
