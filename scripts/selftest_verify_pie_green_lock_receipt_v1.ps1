...
$VerifierPath = Join-Path $RepoRoot "scripts\verify_pie_green_lock_receipt_v1.ps1"
$RunRoot = Join-Path $RepoRoot "runs\pie_green_lock_receipt_verify_selftest"
...
if(-not (Test-Path -LiteralPath $VerifierPath -PathType Leaf)){
  throw "PIE_GREEN_LOCK_RECEIPT_VERIFY_SELFTEST_VERIFIER_MISSING"
}
...
if(@($err).Count -gt 0){
  throw ("PIE_GREEN_LOCK_RECEIPT_VERIFY_SELFTEST_VERIFIER_PARSE_FAIL: " + $err[0].ToString())
}
...
if($P.ExitCode -ne 0){
  throw ("PIE_GREEN_LOCK_RECEIPT_VERIFY_SELFTEST_CHILD_FAIL: exit=" + [string]$P.ExitCode)
}

if($Stdout -notmatch 'PIE_GREEN_LOCK_RECEIPT_VERIFY_OK'){
  throw "PIE_GREEN_LOCK_RECEIPT_VERIFY_SELFTEST_TOKEN_MISSING"
}

if(-not [string]::IsNullOrWhiteSpace($Stderr)){
  throw "PIE_GREEN_LOCK_RECEIPT_VERIFY_SELFTEST_STDERR_NOT_EMPTY"
}
...
if($BeforeJoined -ne $AfterJoined){
  throw "PIE_GREEN_LOCK_RECEIPT_VERIFY_SELFTEST_TRACKED_STATE_CHANGED"
}

$Summary = [ordered]@{
  schema = "pie.green.lock.receipt.verify.selftest.v1"
...
$SummaryPath = Join-Path $CaseRoot "pie_green_lock_receipt_verify_selftest_summary.json"
$LatestSummaryPath = Join-Path $RunRoot "latest_pie_green_lock_receipt_verify_selftest_summary.json"
...
Write-Host "PIE_GREEN_LOCK_RECEIPT_VERIFY_SELFTEST_OK" -ForegroundColor Green
