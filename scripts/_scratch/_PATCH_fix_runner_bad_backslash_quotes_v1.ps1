...
# Fix: replace invalid \" with PowerShell-escaped quote `" (backtick+quote)
$before = $txt
$txt2 = $txt -replace "\\\\\"" , "`"`""
...
Write-Host ("PATCH_OK: fixed invalid backslash-quotes in runner (backup=" + $bak + ")") -ForegroundColor Green
