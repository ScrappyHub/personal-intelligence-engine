...
function PIE_ComputeRunLedgerLine([string]$RepoRoot,[hashtable]$run){ $p=PIE_RunLedgerPath $RepoRoot; $enc=New-Object System.Text.UTF8Encoding($false); $existing=""; $prev=""; if(Test-Path -LiteralPath $p -PathType Leaf){ $existing=[System.IO.File]::ReadAllText($p,$enc); $lines=@(@([System.IO.File]::ReadAllLines($p,$enc))); if($lines.Count -gt 0){ $prev=$lines[$lines.Count-1] } } if(-not [string]::IsNullOrWhiteSpace($prev)){ $run.prev_hash = PIE_Sha256HexBytes ([System.Text.Encoding]::UTF8.GetBytes($prev)) } $line=NL_ToCanonJson $run; [pscustomobject]@{ line=$line; existing=$existing; path=$p } }
