...
if ($Sign) {
  ...
  $envPath = Join-Path $tmp "sig_envelope.v1.json"
  $sigPath = Join-Path $tmp "signatures\sig_envelope.sig"
  if (-not (Test-Path -LiteralPath $envPath -PathType Leaf)) {
    $ls = @(@(Get-ChildItem -LiteralPath $tmp -Recurse -Force | Select-Object -ExpandProperty FullName))
    Die ("sign_requested_but_missing_sig_envelope: " + $envPath + " files=" + ($ls -join ";"))
  }
  if (-not (Test-Path -LiteralPath $sigPath -PathType Leaf)) {
    $ls = @(@(Get-ChildItem -LiteralPath $tmp -Recurse -Force | Select-Object -ExpandProperty FullName))
    Die ("sign_requested_but_missing_signature_file: " + $sigPath + " files=" + ($ls -join ";"))
  }
}
